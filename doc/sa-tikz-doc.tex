...
\newcommand{\versiondate}{December 31, 2012}
...
\begin{key}{/tikz/connections disabled=\mchoice{true,false} (default false)}
	This option, not active by default |connections disabled/.default=false|, allows to remove the connections between the stages when set to \opt{true}. Beware: this option is valid only for |clos snb|, |clos rear|, |benes| and |benes complete| networks.
\end{key}
...
\end{itemize}
But, in the first stage $\gamma=8$, in the second stage $\gamma=4$ and in the third stage $\gamma=2$: this means that $\gamma$ defines the \emph{module applicability range}.
...
\For{$stg\leftarrow 1$ \KwTo $(I_1-1)$}{
	...
	compute $\psi=2^{stg-1}$\;
	\For{$rep\leftarrow 1$ \KwTo $\psi$}{
		compute starting point $x=1+((rep-1)\cdot\gamma)$\;
