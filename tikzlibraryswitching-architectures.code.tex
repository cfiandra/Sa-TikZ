...
\tikzset{omega/.code={
	...
	\foreach \s [evaluate=\s as \numstage using int(\s-1)] in {0,...,\stages}{
		\ifnum\s=0
			...
			\foreach \i in {1,...,\r}{
				\path let \n2={-\i*\moduleysep} in
				  node[rectangle,minimum height=\modulesize,#1,module opacity,xshift=\modulesize/2] 
				...
				\foreach \routput[evaluate=\routput as \rinterval using \rintervalspace*\routput] in {1,...,\m}
					\draw ($(r\s-\i.north east)!\rinterval!(r\s-\i.south east)+(0.5*0.3*\pinlength,0)$) coordinate (r\s-\i-front output-\routput) {} --
					      ($(r\s-\i.north east)!\rinterval!(r\s-\i.south east)$)                        node       (r\s-\i-output-\routput) {};
			}
		\fi
		\ifnum\s=\stages
			% Final Module
			\foreach \i in {1,...,\r}{
				\path let \n1 = {int(0-\i)}, \n2={0-\i*\moduleysep} in node[module,#1,module opacity] (r\s-\i) at 
					+(\numstage*0.6*\modulexsep,\n2) {\pgfmathparse{int(multiply(\n1,-1))}\pgfmathresult};

				% Final module inputs
				\pgfmathsetmacro\roneintervalspace{1/(\m+1)}
				\foreach \roneinput[evaluate=\roneinput as \roneinterval using \roneintervalspace*\roneinput] in {1,...,\m}
					\draw ($(r\s-\i.north west)!\roneinterval!(r\s-\i.south west)-(0.5*0.3*\pinlength,0)$) coordinate                  (r\s-\i-front input-\roneinput) {} --
		...
		\ifnum\cond=1
			% Intermediate modules
			\foreach \i in {1,...,\r}{
				\path let \n1 = {int(0-\i)}, \n2={0-\i*\moduleysep} in node[module,#1,module opacity]  
					(r\s-\i) at +(\numstage*0.6*\modulexsep,\n2) {\pgfmathparse{int(multiply(\n1,-1))}\pgfmathresult};
	...
	\else
		...
		\foreach \stg[evaluate=\stg as \prevstg using int(\stg - 1)] in {1,...,\stages}{
			\foreach \startmod in {1,...,\r}{
				\rotateright{2*(\startmod - 1)}{\stages - 1}{address}
